...
2. **Code hiện có ngày 20/09/2026:** full schema + migration TM-70; Tour/TourSchedule/Destination/TourDestination EF mapping; public `GET /api/v1/tours` lọc bất kỳ vùng đã gắn và trả `destinations[]`; validation/contract/OpenAPI; SQL, endpoint và unit tests. Worktree `Capstone_BE_tm70`, branch `feature/datmnt-search-tours` đã commit TM-70 tại `f5540c0` và merge `origin/develop@930a5f9` tại `2641c2d` (không force-push). Chưa áp migration lên cloud DB.
3. **Bằng chứng hiện tại cho D1 nhiều vùng sau merge develop:** Release build 0 warning/0 error; full SQL-enabled suite trên SQL Server 2022 container test riêng: 8 Infrastructure + 368 Application + 225 API = **601 pass, 0 fail, 0 skip**. SQL-only suite: **41 pass, 0 fail, 0 skip**. Scoped format và NuGet vulnerability scan trước merge đã qua; cần xác minh lại trên head cuối cùng. Bằng chứng cũ 283/28 trước merge và 281/26 của D1 một chuỗi không còn dùng để nghiệm thu head mới. Đây là local evidence, không thay thế CI trên PR head.
...
**Trạng thái:** code Database + Backend cho D1 nhiều vùng đã commit và qua local full SQL-enabled suite sau merge develop; chưa áp migration lên DB cloud hoặc backfill dữ liệu vùng thật. Tình trạng push/PR/CI phải đối chiếu GitHub ở head cuối; không merge PR hoặc cập nhật Jira tự động. Web/Mobile để Phase 2.
